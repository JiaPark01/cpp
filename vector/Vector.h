...
public:
    ...
    Vector();
    explicit Vector(size_type n, const value_type& value = T());  // default. if int, 0
...
private:
    ...
    iterator data;  // start
    iterator avail; // where actually is the last point
    iterator limit; // one after the last(avail)
...
template <typename T>
void Vector<T>::push_back(const Vector<T>::value_type& value)
{
    // check full
